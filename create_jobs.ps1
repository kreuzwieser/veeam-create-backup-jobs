...
# Tomas Kreuzwieser, 2014-2015
#############################################################################################

# source links:
# http://methodicallyaimless.wordpress.com/tag/veeam/
# http://www.vpowercli.co.uk/
# howto modify jobs to forewer incremental: https://gist.github.com/smasterson/9140983
...
$pools_for_exclude = ("smetiste", "replicas", "backup", "sandbox", "View-WIN7", "View-WIN8", "View-FBM", "View-RO", "VIEW", "Vertica")
# number of the job that are running in parallel
$parallel_num = 8
# first VM backup in each of the thread / stream # 19:00:00
$StartTime = ('19:00:00')
# number of backups / restore points
$restore_points_num = 35
...
$PrevJobInStream = @{}
# information
$prog_information = "generated by create veeam jobs script (version 6/2015)" 
# full backup transform period (create every n day)
$sfull_period = 2000; # temporary defacto off for all full backups
# types of backups
$job_types_do = ("Backup");
...
foreach ($job_type in $job_types_do)
{
    ...
    foreach ($vCenterServer in ($vCenterServers))
    {
        ...
        foreach ($vm in (Get-View -ViewType VirtualMachine -Filter @{'Config.Template'='False'} | Sort-Object Name))
        {
            ...
            # temporary force name based exclude
            if (($vm_name -like '*preklad*') -or ($vm_name -like '*shreck*') -or ($vm_name -like '*collector*') -or ($vm_name -like '*wolf*') -or ($vm_name -like '*harrier*'))
            {
                $do_backup = $false
            }
                ...
                { # update objects (VMS) in the backup job, if necessary
                    ...
                    if ($ServerEntity.length -gt 0)
                    {
                        foreach ($obj in $ObjectsInJob)
                        {
                            ...
                            #}
                            if (!(($ServerEntity[0].Uuid -eq $Items.Info.Uuid) -and ($ServerEntity[0].Path -eq $Items.Info.Path) -and ($ServerEntity[0].Name -eq $Items.Info.Name) -and ($ServerEntity[0].Reference -eq $Items.ObjectRef)))
                            {
                                $regenerate_objects = $true
                            }
                        }
