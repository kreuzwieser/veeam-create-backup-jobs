...
$PrevJobInStream = @{}
# information
$prog_information = "generated by create veeam jobs script (TK 2014)" 
...
. c:\scripts\create_jobs_credentials.ps1
        ...
        foreach ($vm in (Get-View -ViewType VirtualMachine -Filter @{'Config.Template'='False'} | Sort-Object Name))
        {
                ...
                foreach ($rp in (Get-ResourcePool -vm $vm.name))
                {
                    ...
                    { # check if VM is in some excluded resource pool
                        ...
                        { # VM is in excluded resource pool, we omit this VM from backup
                            write-host "exclude VM $vm_name from backups (VM is in RP $rp.name)"
                            $do_backup = $false
                        }
                    }
                }
            ...
            { # do backup - VM is not excluded
                ...
                { # set the backup job options
                    ...
                    $JobOptions.BackupStorageOptions.RetainCycles = 180 #This is the number of restore points kept
                    ...
                    $curr_date = Get-Date
                    $full_in_days = [math]::abs(($curr_date.DayOfYear % $sfull_period) - (($hash[14]*16 + $hash[15]) % $sfull_period)) 
                    $next_full = $curr_date.AddDays($full_in_days).ToShortDateString()
                    
                    if ($full_in_days -eq 0)
                    {
                        write-host "Transformation of the backup job to full backup will today"
                        $JobOptions.BackupTargetOptions.TransformToSyntethicDays = $curr_date.DayOfWeek
                    }
                    else
                    {
                        write-host "Transformation of the backup job to full backup will run for $full_in_days ($next_full)"
